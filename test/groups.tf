locals {
  groups = {
    ...
    dataanalysts   = { display_name = "Data Analysts", allow_cluster_create = false, workspace_access = false, databricks_sql_access = false, permission = "USER" }
...
locals {
  schema_group_entitlements = {
    audit_common = { workspace_access = true, databricks_sql_access = true }
  }
}
